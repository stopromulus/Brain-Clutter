...
def complete(note_text):
    ...
    response = openai.Completion.create(model="text-davinci-003", temperature=0.1, max_tokens=128,
                                        prompt="Correct grammar & punctuation, summarize, and extract subject, "
                                               "location, & time (if possible). Then, write a two word summary ("
                                               f"subheading) derived from this statement: {note_text}")
